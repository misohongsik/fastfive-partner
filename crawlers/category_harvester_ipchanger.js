...
// 브라우저 시작 및 재시작 관리 함수 (IP 변경 로직 포함)
...
async function runCategoryHarvester() {
    ...
    // 초기 브라우저 실행 및 로그인 시도 (forceIpChange = false)
    if (!(await initializeBrowser(false))) {
        // 초기 로그인 실패 시 IP를 변경하고 한 번 더 재시도 (forceIpChange = true)
