...
// --- 프록시 및 IP 변경 설정은 모두 제거되었습니다. (로컬 IP 사용) ---
...
// =======================================================================
// ▼▼▼ 브라우저 초기화 및 로그인 관리 (로컬 환경용으로 수정됨) ▼▼▼
// =======================================================================

// [수정됨] 로컬 환경용 브라우저 실행 (프록시 없음)
...
// 브라우저 시작 및 재시작 관리 함수
// [수정됨] IP 변경 로직 제거, 단순 브라우저 재시작 및 로그인 확인
...
async function runCategoryHarvester() {
    ...
    if (!(await initializeBrowser())) {
        // 초기 실패 시 즉시 한 번 더 재시도 (로컬 환경에서는 세션 문제일 수 있음)
        console.log("⚠️ 초기 접속 실패. 재시도합니다.");
        if (!(await initializeBrowser())) {
    ...
    while ((currentCategory = await getNextUnexploredCategory()) !== null) {
        ...
        try {
            ...
            if (isBotCheck) {
                console.log("🚨 봇 탐지(Captcha) 감지됨. 브라우저 재시작.");
                // 재시작 시도 (IP 변경 없음)
                if (!(await initializeBrowser())) {
            ...
            if (error.name === 'TimeoutError' || error.message?.includes('net::ERR')) {
                console.log("🌐 네트워크 오류 감지. 브라우저 재시작.");
                // 재시작 시도 (IP 변경 없음)
                if (!(await initializeBrowser())) {
                    console.error("🛑 브라우저 재시작 실패로 스크립트를 종료합니다.");
                    break;
